...
public class ExecutionFitnessMeasurement extends Measurement {
	private final String NAME = "Frament execution fitness (given it was executed)";
	private double shortest_path_through_frag;

	/**
	 * Only considers the fitness of traces that contain a fragment execution
	 */
	public MeasurementValue evaluate(PetriNetRPSTNode fragment, SyncReplayResult alignment, XTrace trace, PetriNetRPST rpst, TransEvClassMapping mapping) {
		...
		boolean has_execution = false;
		
		AlignmentReplayer replayer = new AlignmentReplayer(rpst.getNet().getNet(), trace, alignment, fragment);
		while (replayer.hasNext()) {
			replayer.next();
			if ((replayer.curStepType().equals(SYNC_MV) || replayer.curStepType().equals(MODEL_MV) || replayer.curStepType().equals(INVISIBLE_MV)) && fragment_trans.contains(replayer.curModelStep())) {
				if (replayer.curStepType().equals(SYNC_MV)) {
					has_execution = true;
					projected_trace_length += 1;
				}
				if (replayer.curStepType().equals(MODEL_MV)) {
					has_execution = true;
					cost += 1;
				}
				if (replayer.curStepType().equals(INVISIBLE_MV)) {
					has_execution = true;
				}
			}
			if (replayer.curStepType().equals(LOG_MV) && fragment_ev_ids.contains(ev_classifier.getClassIdentity(replayer.curLogStep()))) {
				cost += 1;
				projected_trace_length += 1;
			}
		}
		...
		if (!has_execution) return null; // alignment does not execute the fragment
